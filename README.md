# 📚 algorithm-learning-journey
> 我的算法与数据结构公开学习日志 | 从《算法竞赛入门经典（第2版）》第6章开始，每日更新

## 🎯 学习目标
- 系统掌握算法竞赛核心知识点，冲刺蓝桥杯省一/国赛
- 吃透《算法竞赛入门经典》全册，构建完整算法知识体系
- 每日刷题+总结，形成可追溯的学习成长轨迹

## 📖 学习进度（《算法竞赛入门经典 第2版》）
| 章节 | 学习状态 | 完成时间 |
| :--- | :--- | :--- |
| 第1-5章 C++基础 | ✅ 已学完 | 2026-04-14 |
| 第6章 数据结构基础 | 📖 学习中 | - |
| 第7章 暴力求解法 | ⏳ 待学习 | - |
| 第8章 高效算法设计 | ⏳ 待学习 | - |
| 第9章 动态规划初步 | ⏳ 待学习 | - |
| 第10章 数学概念与方法 | ⏳ 待学习 | - |
| 第11章 图论算法与模型 | ⏳ 待学习 | - |
| 第12章 高级专题 | ⏳ 待学习 | - |

## 📁 仓库目录结构
```
.
├── code/ # 所有刷题代码、手写数据结构实现
│ ├── chapter6/ # 第 6 章对应代码
│ ├── chapter7/ # 第 7 章对应代码
│ └── ...
├── notes/ # 每日学习笔记、知识点总结、错题复盘
├── images/ # 解题思路图、学习截图
├── 基础篇/
│ ├── 6.数据结构基础
│ └── 7.暴力求解法
├── 竞赛篇/
│ ├── 8.高效算法设计
│ ├── 9.动态规划初步
│ ├── 10.数学概念及方法
│ ├── 11.图论模型与算法
│ └── 12.高级专题
├── .gitignore # 忽略编译产物、临时文件
└── README.md # 仓库总览
```
## 📅 每日打卡日志模板
> 按日期记录每日学习内容、刷题成果、掌握情况

### 2026-00-0 Day0
✅ **今日学习内容**
- 

💻 **今日刷题/代码**
- [洛谷 P1739 表达式括号匹配](sslocal://flow/file_open?url=https%3A%2F%2Fwww.luogu.com.cn%2Fproblem%2FP1739&flow_extra=eyJsaW5rX3R5cGUiOiJjb2RlX2ludGVycHJldGVyIn0=) ✅ AC
- 代码文件：[code/chapter6/stack_bracket_match.cpp](sslocal://flow/file_open?url=code%2Fchapter6%2Fstack_bracket_match.cpp&flow_extra=eyJsaW5rX3R5cGUiOiJjb2RlX2ludGVycHJldGVyIn0=)
- 手写栈实现：[code/chapter6/stack_manual.cpp](sslocal://flow/file_open?url=code%2Fchapter6%2Fstack_manual.cpp&flow_extra=eyJsaW5rX3R5cGUiOiJjb2RlX2ludGVycHJldGVyIn0=)

📈 **掌握程度**
- ✅ 栈原理：完全掌握/初步了解/仍然不理解
- ✅ 括号匹配：完全掌握/初步了解/仍然不理解
- ⚠️ 手写栈边界条件：需巩固

🚀 **明日计划**
- 学习队列（Queue）原理与实现
- 完成洛谷 P1996 约瑟夫问题
- 掌握STL `std::queue` 使用

---

## 🤝 交流与反馈
- 欢迎在 Issues 交流学习心得、讨论题目
- 我的掘金/CSDN：待填写
- 联系方式：xjj221439@gmail.com

## 📜 开源协议
本仓库采用 MIT License 开源
